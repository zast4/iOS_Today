...
class ReminderListViewController: UICollectionViewController {
    ...
    private func listLayout() -> UICollectionViewCompositionalLayout {
        ...
        listConfiguration.trailingSwipeActionsConfigurationProvider = makeTrailingSwipeActions(for:)
        listConfiguration.leadingSwipeActionsConfigurationProvider = makeLeadingSwipeActions(for:)
    ...
    private func makeLeadingSwipeActions(for indexPath: IndexPath?) -> UISwipeActionsConfiguration? {
        guard let indexPath else {
            return nil
        }
        
        let editActionTitle = NSLocalizedString("Edit", comment: "Edit action title")
        let editAction = UIContextualAction(style: .normal, title: editActionTitle) { [weak self] _, _, completion in
            guard let self else { return }
            let reminder = self.reminders[indexPath.item]
            
            presentEditScreen(with: reminder)
            completion(false)
        }
